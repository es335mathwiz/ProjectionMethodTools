...
public class StateVarTime extends VarTime {

	public StateVarTime(String aName,int aTime){
		setVarName(aName);
		setVarTime(aTime);
	}
	public StateVarTime(String aName){
		setVarName(aName);
		setVarTime();
	}
	...
	public void setVarTime(){
		setEvaluationTimeAndType(
				VarTimeType.newVarTimeType(-1,
						VarTimeType.getValueAtNode(),VarTimeType.getNOTDRV()));
	}
	...
	throws ProjectionRuntimeException{
		...
		if(theDrvTime==theTime-1){
			if(model.getTheState().getTheGrid().hasVarQ(this)) {
				...
				{
					throw new ProjectionRuntimeException("StateVarTime: only functional derivs of functions mapping t-1 state t or state at t t to t+1 implemented");
				}
	}	
