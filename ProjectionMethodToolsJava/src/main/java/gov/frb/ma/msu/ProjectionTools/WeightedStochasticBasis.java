...
// TODO move all the static methods to a separate class
	...
	public double [][] [] solveParamsRange(double [][] initWts, DoEqns theSys, double [] initParamVec,
			...
			receive=fZero.findCollocationWeights(this,initWts,theSys);
			...
			receive=improveGuess(fZero.findCollocationWeights(this,initWts,theSys),receive);
			 newtonItersSeq.addAll(fZero.getStrategyItersSeq());
			
			} else {
								receive=improveGuess(fZero.findCollocationWeights(this,initWts,theSys),theRes[ii-1]);
								 newtonItersSeq.addAll(fZero.getStrategyItersSeq());
			}
