...
public class NonStateFutureVarTime extends NonStateVarTimeType {
	...
	EquationValDrv doValSwitch(StochasticBasis model,int varNum	){
		  ...
		  int nonStateWtDim=numPolys*nonStateVarDim;
		  int fullWtDim=model.getNumStatePlusNonStatePolys(this);
Matrix		  nxtMat=model.getTheState().getVariablesIteratedFromChebNodesTimeTP1NSP();
Matrix nxtDrvMat= model.getTheState().getVariablesIteratedFromChebNodesTimeTP1DerivWRTWtsNSP();
...
int stateWtDim=nodeDim*stateVarDim;
		  ...
		  val = nxtMat.getMatrix(varNum*numPolys,(varNum+1)*numPolys-1,0,0);
		  JJ=
				 nxtDrvMat.getMatrix(stateWtDim+varNum*numPolys,stateWtDim+(varNum+1)*numPolys-1,0,fullWtDim-1);
				 if(JJ.getColumnDimension()<fullWtDim)JJ=rightAugZeros(JJ,nonStateWtDim);
		newVals=new EquationValDrv(val,JJ);
	...
	EquationValDrv doDrvSwitchNonStateCorrect(Basis model,VarTime theDrvVar,String varName){
		Matrix val;Matrix JJ;
		int varNum = model.getVarNumNSP(this,varName);
